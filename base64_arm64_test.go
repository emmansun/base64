...
func TestStdEncodeSIMD(t *testing.T) {
	pairs := []testpair{
		...
		{"abcdefghijklabcdefghijklabcdefghijklabcdefghijklabcdefghijklabcdefghijklabcdefghijklabcdefghijklabcdefghijklabcdefghijklabcdefghijklabcdefghijklabcdefghijklabcdefghijklabcdefghijklabcdefghijkl", ""},
...
func TestURLDecodeSIMDNibbleDispatch(t *testing.T) {
	old := useURLNibbleDecode
	useURLNibbleDecode = true
	defer func() {
		useURLNibbleDecode = old
	}()

	src := "IT8kKiYoKSctPUB-S_fMJwH-Q5e0nr7t"
	expected := []byte("!?$*&()'-=@~K\xf7\xcc'\x01\xfeC\x97\xb4\x9e\xbe\xed")
	dst := make([]byte, URLEncoding.DecodedLen(len(src)))

	n, err := URLEncoding.Decode(dst, []byte(src))
	if err != nil {
		t.Fatalf("decode failed: %v", err)
	}
	if n != len(expected) {
